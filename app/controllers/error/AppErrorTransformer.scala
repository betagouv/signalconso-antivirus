...
object AppErrorTransformer {
  ...
  private def formatMessage[R <: Request[_]](request: R, maybeUser: Option[UUID], appError: ApiError): String =
  ...
  private def handleAppError[R <: Request[_]](request: R, error: ApiError, maybeUserId: Option[UUID]): Result =
    error match {
